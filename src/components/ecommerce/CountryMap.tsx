...
const CountryMap: React.FC<CountryMapProps> = ({ mapColor }) => {
  ...
  return (
    <div className="w-full h-[350px] mx-auto m-20 flex items-center justify-center relative">
      {loading ? (
        // ✅ Loader (spinner)
        ...
        <VectorMap
          ...
          onMarkerTipShow={(_e, el, index: string) => {
            ...
            const marker = markers[markerIndex];
            if (!marker) return;
            const country = Object.values(countryInfo).find(
              (c) => c.country === marker.name
            );
            if (country) {
              (el as unknown as JVectorElement).html(
                `${country.country} ( Coaches:${country.coaches}, Players:${country.players})`
